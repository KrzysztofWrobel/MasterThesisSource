...
bool Solve5PointEssential(double *pts1, double *pts2, int num_pts, std::vector<cv::Mat> &ret_E, std::vector<cv::Mat> &ret_P, std::vector<int> &ret_inliers)
{
    ...
    for(size_t i=0; i < solutions.size(); i++) {
        ...
        x1.at<double>(0,0) = pts1[0];
        x1.at<double>(1,0) = pts1[1];
        x1.at<double>(2,0) = 1.0;

        x2.at<double>(0,0) = pts2[0];
        x2.at<double>(1,0) = pts2[1];
        ...
        for(int j=0; j < 4; j++) {
            ...
            if(depth1 > 0 && depth2 > 0){
                ...
                for(int k=1; k < num_pts; k++) {
                    pt3d = TriangulatePoint(cv::Point2d(pts1[k*2], pts1[k*2+1]), cv::Point2d(pts2[k*2], pts2[k*2+1]), P_ref, P[j]);
